...
What we care primarily about in this tutorial is the vast amount of historical data that Tradier offers through its API. Before this, it would have taken a deal with an exchange or some really roundabout and/or expensive path to get access to this kind of data, which has historically made it difficult for average investors to do the kinds of analyses that the professionals do. Yahoo had YQL, or Yahoo Query Language, that attempted to solve this problem, but it is a bit unwieldy to work with. Moreover, it doesn't seem supported that well. It's pretty impressive that we have access to a vast amount of historical stock data through a simple Tradier API.
